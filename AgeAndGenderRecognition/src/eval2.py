...
def eval_once(saver, summary_writer, summary_op, logits, labels, num_eval, requested_step=None):
    ...
    """
    top1Age = tf.nn.in_top_k(logits[0], labels[0], 1)
    top2Age = tf.nn.in_top_k(logits[0], labels[0], 2)

    top1Gender = tf.nn.in_top_k(logits[1], labels[1], 1)
    top2Gender = tf.nn.in_top_k(logits[1], labels[1], 2)
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            summary.value.add(tag='Precision @ 1', simple_value=at1Age)
            summary.value.add(tag='   Recall @ 2', simple_value=at1Age)
...
def evaluate(run_dir):
    with tf.Graph().as_default() as g:
        ...
        with tf.device(FLAGS.device_id):
            ...
            logits = model_fn([md['nlabels1'], md['nlabels2']], images, 1, False)
