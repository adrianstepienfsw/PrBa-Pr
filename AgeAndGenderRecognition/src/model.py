...
def select_model(name):
    if name.startswith('inception'):
        ...
        print('selected common model')
        return levi_hassner_common
...
def levi_hassner_common(nlabels, images, pkeep, is_training):
    weight_decay = 0.0005
    weights_regularizer = tf.contrib.layers.l2_regularizer(weight_decay)
    with tf.variable_scope("LeviHassner", "LeviHassner", [images]) as scope:
        with tf.contrib.slim.arg_scope(
    ...
    with tf.variable_scope('output') as scope:
        weightsAge = tf.Variable(tf.random_normal([512, nlabels[0]], mean=0.0, stddev=0.01), name='weightsAge')
        biasesAge = tf.Variable(tf.constant(0.0, shape=[nlabels[0]], dtype=tf.float32), name='biasesAge')
        outputAge = tf.add(tf.matmul(drop2, weightsAge), biasesAge, name='output1')
        weightsGender = tf.Variable(tf.random_normal([512, nlabels[1]], mean=0.0, stddev=0.01), name='weights')
        biasesGender = tf.Variable(tf.constant(0.0, shape=[nlabels[1]], dtype=tf.float32), name='biases')
        outputGender = tf.add(tf.matmul(drop2, weightsGender), biasesGender, name='output2')
    return [outputAge, outputGender]
