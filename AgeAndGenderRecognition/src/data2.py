...
def parse_example_proto(example_serialized):
  ...
  feature_map = {
      ...
      'image/class/labelAge': tf.FixedLenFeature([1], dtype=tf.int64,
                                              default_value=-1),
      'image/class/labelGender': tf.FixedLenFeature([1], dtype=tf.int64,
                                               default_value=-1),
  ...
  labels = [tf.cast(features['image/class/labelAge'], dtype=tf.int32), tf.cast(features['image/class/labelGender'], dtype=tf.int32)]
...
def batch_inputs(data_dir, batch_size, image_size, train, num_preprocess_threads=4,
  ...
  with tf.name_scope('batch_processing'):
    ...
    for thread_id in range(num_preprocess_threads):
      ...
      images_labels_fnames.append([image, label_index[0], label_index[1], fname])

    images, label_index_batch1, label_index_batch2, fnames = tf.train.batch_join(
        images_labels_fnames,
    ...
    return images, [tf.reshape(label_index_batch1, [batch_size]), tf.reshape(label_index_batch2, [batch_size])], fnames
