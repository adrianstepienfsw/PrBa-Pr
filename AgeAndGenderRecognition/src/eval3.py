    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while step < num_steps and not coord.should_stop():
                ...
                v, lAge = sess.run([tf.math.sigmoid(logits), labels])
                ...
                v[v!=1] = 0
                v = np.sum(v, axis=1)
                lAge = np.sum(lAge, axis=1)
                true_count1Age += np.sum(v==lAge)
                true_count2Age += np.sum(np.abs(v-lAge)<=1)
